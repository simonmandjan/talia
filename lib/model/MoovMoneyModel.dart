class MoovCollectionResponse {
  final bool success;
  final String? message;
  final String? transactionId;
  final String? status;

  MoovCollectionResponse({
    this.success = false,
    this.message,
    this.transactionId,
    this.status,
  ...
  factory MoovCollectionResponse.fromJson(Map<String, dynamic> json) {
  ...
  bool get isFinalFailure {
    ...
    // Anything else (Moov's raw pending code, an unrecognized status, or a
    // malformed/missing status block alongside a real transaction) is
    // ambiguous, not a confirmed failure — the polling loop keeps going,
    // bounded by its own max-attempts timeout, so defaulting to "keep
    // polling" here is safe.
    return false;
  }
}
